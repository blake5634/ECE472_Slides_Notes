...
Here we show the amplifier input as idealized (drawing no current from the signal).  But,
we also have a $1M\Omega$
resistor in parallel with an $18pF$ capacitor connected to ground.
These represent the non-ideal
electrical properties of the actual scope input
...
Now we can look at the voltage divider approximating $Z_{Th} \approx 0$ (where
 we {\bf assume} that the new impedance
...
$Z_2$ (the 10x probe).   If you look at the connector body of your probe, you can see a little hole.

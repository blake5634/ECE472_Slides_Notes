...
def patch_preamble(text: str, prefix: str, title: str) -> str:
    ...
    if count == 0:
        # Fallback: just replace bare \usepackage{hyperref}
        bare_pat = re.compile(
            r'(' + re.escape(prefix) + r'\\usepackage\{hyperref\})',
            re.MULTILINE)
        new_text = bare_pat.sub(hyperref, text, count=1)
    return new_text
...
def generate_html(shn_path: Path, stream: str = HTML_STREAM):
    ...
    # Step 1: generate .tex from .shn
    run(['coursetex', '--out', stream, str(shn_path)])
